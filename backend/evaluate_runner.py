...
from .prompts.evaluate import (
    build_evaluation_synthesis_prompt,
...
async def run_evaluate_case(
    case_input: CaseInput,
    run_id: str | None = None,
) -> RunRecord:
    """Run independent evaluations, synthesize them, and persist the run."""
    ...
    try:
        evaluator_prompt = build_evaluator_prompt(case_input)
        ...
        synthesis_prompt = build_evaluation_synthesis_prompt(case_input, evaluations)
        ...
        if not evaluations:
            status = "failed"
            errors.append("No evaluator responses were collected")
